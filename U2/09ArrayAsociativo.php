...
<body>
    ...
    if(isset($_GET['crearArray'])){
        //Crear un array asociativo vacío para guadar los datos del formulario
        $datos = array();
        //Añadir el nombre
        $datos['miNombre']= $_GET['nombre'];
        //Añadir edad
        $datos['miEdad']= $_GET['edad'];
        //Añadir estatura
        $datos['miEstatura']= $_GET['estatura'];
        //Añadir curso
        $datos['miCurso']= $_GET['curso'];
        //Añadir color
        $datos['miColor']= $_GET['color'];
        //Añadir Centro
        $datos['miCentro']='IES AUGUSTÓBRIGA';
        //aÑADIR Cp
        $datos['miCP']=10300;

        //Añadir al array 5 nºs aleatorios
        for($i=0;$i<5;$i++){
            $datos['miAleatorio'.$i+1]=rand();
        }
